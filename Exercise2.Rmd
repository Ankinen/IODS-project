...
After the c orrect function has been identified and data read into R, the data will usually be in R `data.frame` format. Te dimensions of a data frame are ($n$,$d$), where $n$ is the number of rows (the observations) and $d$ the number of columns (the variables).
...
Sometimes you want to rename your columns. You could do this by creating copies of the columns with new names, but you can also directly get and set the column names of a data frame, using the function `colnames()`.
...
- Adjust the plot initialization: 
...
# add col=gender inside aes()
p1 <- ggpairs(learning2014, mapping = aes(col = gender), lower = list(combo = wrap("facethist", bins = 20)))

# draw the plot again
p1

# add alpa = 0.3 inside aes()
p2 <- ggpairs(learning2014, mapping = aes(col = gender), lower = list(combo = wrap("facethist", bins = 20)))

# draw the plot once more
p2
```
...
# adjust the code
# add a third variable, choose the variable with the third highest correlation
